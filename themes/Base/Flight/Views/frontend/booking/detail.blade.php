            ...
            <div class="service-info">
                <div>
                    @if ($image_url = $service->airline->image_url)
                        ...
                        @else
                            {!! get_image_tag($service->airline->image_id, 'medium', [
                    ...
                    <div class="flex-self-start">
                        ...
                        <div class="text-lh-sm ml-1">
                            <h6 class="font-weight-bold font-size-21 text-gray-5 mb-0">
                                {{ $service->departure_time->format('H:i') }}</h6>
                            <span
                    ...
                    <div class="text-center d-none d-md-block d-lg-none">
                        <div class="mb-1">
                            <h6 class="font-size-14 font-weight-bold text-gray-5 mb-0">
                                {{ __(':duration hrs', ['duration' => $service->duration]) }}</h6>
                        </div>
                    </div>
                    <div class="flex-self-start">
                        <div class="mr-2">
                        ...
                        <div class="text-lh-sm ml-1">
                            <h6 class="font-weight-bold font-size-21 text-gray-5 mb-0">
                                {{ $service->arrival_time->format('H:i') }}</h6>
                            <span
                ...
                @if (!empty($flight_seat))
                    @foreach ($flight_seat as $type)
                        @if (!empty($type['number']))
                            <li>
                                <div class="label">{{ __('Flight') }}</div>
                                <div class="val">{{ $service->title }}</div>
                            </li>
                            <li>
                                <div class="label">{{ __('PNR') }}</div>
                                <div class="val">{{ $service->pnr }}</div>
                            </li>
                            <li>
                                <div class="label">{{ __('From') }}</div>
                                <div class="val">{{ $service->airportFrom->name }}</div>
                                <p>{{ $service->departure_time->format('H:i') }}</p>
                            </li>
                            <li>
                                <div class="label">{{ __('To') }}</div>
                                <div class="val">{{ $service->airportTo->name }}</div>
                                <p>{{ $service->arrival_time->format('H:i') }}</p>
                            </li>
                            <li>
