...
?>
<h4 class="form-section-title">{{ __('Tickets / Traveller Information:') }}</h4>
        ...
        <div class="card">
            <div class="card-header c-pointer" id="passenger_heading_{{ $i + 1 }}">
                <h4 class="mb-0 " style="font-size: 16px" data-toggle="collapse"
                    data-target="#passenger_{{ $i + 1 }}" data-bs-toggle="collapse"
                    data-bs-target="#passenger_{{ $i + 1 }}" aria-expanded="true"
                    aria-controls="passenger_{{ $i + 1 }}">
                    {{-- @php
                        print_r($passenger);
                    @endphp --}}
                    {{ __('Traveller #:number', ['number' => $i + 1]) }}: {{ $passenger->first_name }}
                    {{ $passenger->last_name }}
                </h4>
            </div>
                        ...
                        <div class="col-md-6">
                          ...
                          <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <strong> {{ $passenger->age_group }}</strong>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
