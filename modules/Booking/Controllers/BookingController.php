...
class BookingController extends \App\Http\Controllers\Controller
{
    ...
    protected function savePassengers(Booking $booking, Request $request)
    {
        ...
        // Gather all input passengers in order: main passenger + extras
        $allPassengersInput = [];
        ...
        $extraPassengersInput = $request->input('extra_customers', []);
        foreach ($extraPassengersInput as $extra) {
            $allPassengersInput[] = [
        ...
        // Helper to check if passenger data is not empty
        $hasValidData = function ($data) {
            return !empty($data['first_name']) || !empty($data['last_name']) || !empty($data['email']) || !empty($data['phone']);
        };

        // Update existing passengers or create new ones if not enough rows exist
        foreach ($allPassengersInput as $index => $passengerData) {
            if (!$hasValidData($passengerData)) {
                // skip empty passengers
                continue;
            }

            // Apply service filter if applicable
            if ($booking->service && method_exists($booking->service, 'filterPassengerData')) {
                $passengerData = $booking->service->filterPassengerData($passengerData, $booking, $request, $index);
            }

            if (isset($passengers[$index])) {
                // Update existing passenger
                $passenger = $passengers[$index];
                $passenger->fillByAttr(array_keys($passengerData), $passengerData);
                $passenger->save();
            } else {
        ...
        // If there are more existing passengers than input, delete the extras
        if (count($passengers) > count($allPassengersInput)) {
            for ($i = count($allPassengersInput); $i < count($passengers); $i++) {
                $passengers[$i]->delete();
            }
        }
    }
